...
pub_plot_1 <- ggplot(df_asp_imp_pub, aes(x = es, y = reorder(unique_label, desc(plot_order)), color = study)) +
  ...
  geom_hline(yintercept = c(3.5,6.5,9.5,12.5), linetype = 2)+
  geom_text(aes(label = text_label, x = 1.20 * max(ci.hi)), hjust = 0, size = 4.5, nudge_x = 0.02, colour = "black")+
  xlim(-.5,.5)
```
...
pub_plot_comp
pub_plot_comp%>%
  ggsave(filename="pub_plot_comp.jpg",width = 40, height = 30)
```

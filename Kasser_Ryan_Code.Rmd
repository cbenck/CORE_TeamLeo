...
ggplot(results_ttest_plot_data, aes(x = d, y = Label, color = dimension)) +
  geom_point() +  # Plot the correlation coefficients as points
  geom_errorbarh(aes(xmin = d_CI_low, xmax = d_CI_high), height = 0.2) +  # Draw horizontal error bars for CIs
  geom_text(aes(label = text_label, x = 1.05 * max(d_CI_high)), hjust = 0, size = 3, nudge_x = 0.02, colour = "black") +
  facet_wrap(~dimension, scales = "free_y", ncol = 1) +  # Group by 'dimension'
  theme_minimal() +  # Use a minimal theme for a clean look
  labs(x = "Correlation Coefficient (r)", y = "") +  # Label the x-axis
  theme_classic()+
...
#exclude importances (aspiration)
df_coded <- df_coded %>%
...
# Assuming the dataframe is named test
...
# Step 2: Plotting
ggplot(test, aes(x = es, y = reorder(unique_label, desc(plot_order)), color = study)) +

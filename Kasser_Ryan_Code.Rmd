...
#money
correlation_gpr_money <- df_means_scaled %>%
...
correlation_gpr_money  <- correlation_gpr_money[1:5,]
correlation_gpr_money <- as.data.frame(cbind(correlation_gpr_money$Parameter1,
                                      correlation_gpr_money$Parameter2,
                                      round(correlation_gpr_money$r,3),
                                      round(correlation_gpr_money$CI_low,3),
                                      round(correlation_gpr_money$CI_high,3),
                                      round(correlation_gpr_money$p,3)))

names(correlation_gpr_money) <- names_corr

#family security
correlation_gpr_family <- df_means_scaled %>%
...
correlation_gpr_family  <- correlation_gpr_family[1:5,]
correlation_gpr_family <- as.data.frame(cbind(correlation_gpr_family$Parameter1,
                                      correlation_gpr_family$Parameter2,
                                      round(correlation_gpr_family$r,3),
                                      round(correlation_gpr_family$CI_low,3),
                                      round(correlation_gpr_family$CI_high,3),
                                      round(correlation_gpr_family$p,3)))

names(correlation_gpr_family) <- names_corr

#global welfare
correlation_gpr_global_welfare <- df_means_scaled %>%
                                select(c(mean_gpr_global_welfare,mean_self_actualization,mean_vitality,
                                mean_control_orientation,mean_depressive_symptoms,mean_trait_anxiety))%>%
                                correlation::correlation(p_adjust = "none")

correlation_gpr_global_welfare  <- correlation_gpr_global_welfare[1:5,]
correlation_gpr_global_welfare <- as.data.frame(cbind(correlation_gpr_global_welfare$Parameter1,
                                      correlation_gpr_global_welfare$Parameter2,
                                      round(correlation_gpr_global_welfare$r,3),
                                      round(correlation_gpr_global_welfare$CI_low,3),
                                      round(correlation_gpr_global_welfare$CI_high,3),
                                      round(correlation_gpr_global_welfare$p,3)))

names(correlation_gpr_global_welfare) <- names_corr
```
...
```{r}
#correlation results
results_corr_gpr <- rbind(correlation_gpr_money,
                          correlation_gpr_family,
                          correlation_gpr_global_welfare)

#remove non-needed dataframes

rm(list = c("correlation_gpr_money",
            "correlation_gpr_family",
            "correlation_gpr_global_welfare"))

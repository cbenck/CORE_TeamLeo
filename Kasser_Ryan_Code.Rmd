...
calculate_group_sd <- function(data, group_var, numeric_var) {
  ...
  # Calculate SDs and group sizes, then pivot to get them in one row
  group_stats <- data %>%
    group_by(!!sym(group_var)) %>%
    summarise(
      SD = sd(!!sym(numeric_var), na.rm = TRUE),
      Size = n(),  # Calculate group size
      .groups = 'drop'
    ) %>%
    pivot_wider(
      names_from = !!sym(group_var),
      values_from = c(SD, Size),
      names_sep = "_"
    )
...
###### only tests with financial from the current data ######

df_asp_imp_pub <- subset(df_asp_imp_all, study == 3)
#modify label
df_asp_imp_pub$unique_label <- substr(df_asp_imp_pub$unique_label, 1, nchar(df_asp_imp_pub$unique_label) - 3)

pub_plot_1 <- ggplot(df_asp_imp_pub, aes(x = es, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci.lo, xmax = ci.hi), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
  ...
  geom_hline(yintercept = c(3.5,6.5,9.5,12.5), linetype = 2, linewidth = 3)+
  geom_text(aes(label = text_label, x = 1.20 * max(ci.hi)), hjust = 0, size = 12.5, nudge_x = 0.02, colour = "black")+
  xlim(-.5,.6)
```
...
df_asp_cha_pub <- subset(df_asp_cha_all, study == 3)
#modify label
df_asp_cha_pub$unique_label <- substr(df_asp_cha_pub$unique_label, 1, nchar(df_asp_cha_pub$unique_label) - 3)

pub_plot_2 <- ggplot(df_asp_cha_pub, aes(x = es, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci.lo, xmax = ci.hi), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
...
df_asp_gp_pub <- subset(df_asp_gp_all, study == 3)
#modify label
df_asp_gp_pub$unique_label <- substr(df_asp_gp_pub$unique_label, 1, nchar(df_asp_gp_pub$unique_label) - 3)

pub_plot_3 <- ggplot(df_asp_gp_pub, aes(x = es, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci.lo, xmax = ci.hi), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
  ...
  geom_hline(yintercept = c(2.5,4.5,6.5,8.5), linetype = 2, linewidth = 3)+
  geom_text(aes(label = text_label, x = 1.20 * max(ci.hi)), hjust = 0, size = 12.5, nudge_x = 0.02, colour = "black")+
  xlim(-.5,.6)

pub_plot_comp<-ggarrange(pub_plot_1, pub_plot_2 , pub_plot_3,
                              font.label = list(size = 70),
                             labels = c("A", "B", "C"),ncol = 2, nrow =2,common.legend = FALSE)

pub_plot_comp
pub_plot_comp%>%
  ggsave(filename="pub_plot_comp.jpg",width = 45, height = 40)
```
...
corr_importance_pub <- subset(corr_importance, study == 3)
#modify label
corr_importance_pub$unique_label <- substr(corr_importance_pub$unique_label, 1, nchar(corr_importance_pub$unique_label) - 3)

corr_importance_pub <- corr_importance_pub[grep("fsi $", corr_importance_pub$unique_label),]

pub_plot_4 <- ggplot(corr_importance_pub, aes(x = r, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci_low, xmax = ci_high), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
  theme_classic(base_size = 70) +
  theme(
  ...
  geom_text(aes(label = text_label, x = 1.20 * max(ci_high)), hjust = 0, size = 12.5, nudge_x = 0.02, colour = "black")+
  xlim(-.5,.6)
...
corr_chance_pub <- subset(corr_chance, study == 3)
#modify label
corr_chance_pub$unique_label <- substr(corr_chance_pub$unique_label, 1, nchar(corr_chance_pub$unique_label) - 3)

corr_chance_pub <- corr_chance_pub[grep("fsc $", corr_chance_pub$unique_label),]

pub_plot_5 <- ggplot(corr_chance_pub, aes(x = r, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci_low, xmax = ci_high), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
...
corr_gpr_pub <- subset(corr_gpr, study == 3)
#modify label
corr_gpr_pub$unique_label <- substr(corr_gpr_pub$unique_label, 1, nchar(corr_gpr_pub$unique_label) - 3)


pub_plot_6 <- ggplot(corr_gpr_pub, aes(x = r, y = reorder(unique_label, desc(plot_order)), color = study)) +
  geom_point(size = 5) + # Plot the effect sizes as points
  geom_errorbarh(aes(xmin = ci_low, xmax = ci_high), height = 0.2, size = 3) + # Add error bars for confidence intervals
  scale_color_viridis_d(begin = 0.2, end = 0.8, name = "Study", guide = guide_legend(override.aes = list(size=4))) +
    labs(x = "", y = "") +
...
pub_plot_corr<-ggarrange(pub_plot_4, pub_plot_5 , pub_plot_6,
                              font.label = list(size = 70),
                             labels = c("A", "B", "C"),ncol = 2, nrow =2,common.legend = FALSE)

pub_plot_corr
pub_plot_corr%>%
  ggsave(filename="pub_plot_corr.jpg",width = 45, height = 20)
```
...
df_corr <- df_corr[grep("(fsc|fsi|money)$", df_corr$Label), ]

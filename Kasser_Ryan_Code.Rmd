...
df_SA <- df_means_scaled %>% select(c(mean_self_actualization, #create specific df for analysis
...
m1 <- lm(mean_self_actualization ~.,data = df_SA) #compute model

standardized_betas <- coef(m1) # Extract coefficients

standardized_cis <- confint(m1, level = 0.95) # Extract confidence intervals

# This step adjusts the CI matrix to have row names matching those of the coefficients
standardized_cis <- setNames(as.data.frame(standardized_cis), c("Lower", "Upper"))
rownames(standardized_cis) <- names(standardized_betas)

summary_m1 <- summary(m1) # Extract p-values
p_values <- summary_m1$coefficients[, "Pr(>|t|)"]

results <- data.frame(Estimate = standardized_betas, row.names = names(standardized_betas)) # Create a data frame for coefficients and their names
...
# Check for any misalignment and adjust
results <- na.omit(results)  # This means that lines without estimates/p-values are dropped (this is probably only important for the simulated data)

# Ensure all components are aligned properly
results_SA <- results

names(results_SA) <- names_regr #rename dataframe with extracted results

#get correlations

correlation_SA <- df_SA %>%
  correlation::correlation(p_adjust = "none")

correlation_SA  <- correlation_SA [1:5,]
correlation_SA <- as.data.frame(cbind(correlation_SA$Parameter1,
                                      correlation_SA$Parameter2,
                                      round(correlation_SA$r,3),
                                      round(correlation_SA$CI_low,3),
                                      round(correlation_SA$CI_high,3),
                                      round(correlation_SA$p,3)))

names(correlation_SA) <- names_corr
